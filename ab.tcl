########################## TCL Event Handlers ##########################
#
#  TNCAC.tcl - 5_axis_dual_table
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Tables.
#
#  Created by 86181 @ Tuesday, December 12 2023, 11:21:07 +0800
...
#
  ...
  set mom_sys_alt_unit_post_name                "TNCAC__IN.pui"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   if { [PB_CMD__check_block_tool_change] } {
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   MOM_force Once G_motion G_mode X Y
...
#=============================================================
   ...
   if { [PB_CMD__check_block_linear_move] } {
...
#=============================================================
  ...
  global mom_sys_machine_time mom_machine_time
   ...
   MOM_output_literal "(Path_name:$mom_operation_name)"
...
#=============================================================
   ...
   PB_CMD_init_helix
   PB_CMD_custom_command_13
   MOM_set_seq_off

   MOM_do_template rewind_stop_code
   PB_CMD_custom_command_7
...
#=============================================================
proc PB_CMD__check_block_linear_move { } {
#=============================================================
global flagx flagy


if { [info exists flagx ]  && $flagx == "ok" } {
unset flagx
set flagy "ok"
return 0
} else {
...
#=============================================================
proc PB_CMD__check_block_tool_change { } {
#=============================================================
...
#
  ...
  if {![string compare "Hole Making" $mom_operation_type] || ![string compare "Point to Point" $mom_operation_type] || \
      ...
      ![string compare "Generic Feature Operation" $mom_operation_type] || ![string compare "Radial Groove Milling" $mom_operation_type]} {
     return 1
  } else {
     return 0
  }
...
# "direct_change" - current motion is non-cutting motion, work plane is changed, there is only one non-cuttong motion between current work plane to next.
...
# "in_change" - current motion is non-cutting motion, there are more than one non-cutting motion between two different work plane, this is one of them but not first one.
...
#=============================================================
  ...
  if { $dpp_ge(ncm_work_plane_change_mode) == "direct_change"} {
        ...
        MOM_output_text "god 01:$dpp_ge(ncm_work_plane_change_mode)"
  } elseif { $dpp_ge(ncm_work_plane_change_mode) != "None"} {
     if { $dpp_ge(ncm_work_plane_change_mode) == "start_change"} {

        MOM_output_text "god 02:$dpp_ge(ncm_work_plane_change_mode)"
         MOM_reload_variable -a mom_pos
         MOM_force once G_adjust H X Y Z fourth_axis fifth_axis
      }
        MOM_output_text "god 03:$dpp_ge(ncm_work_plane_change_mode)"

  } else {
        global mom_nxt_event mom_nxt_motion_event  mom_nxt_motion_type
        global mom_motion_event
        global mom_out_angle_pos mom_prev_out_angle_pos
        if { ![info exists mom_prev_out_angle_pos] } { return }

        if { ![EQ_is_equal $mom_out_angle_pos(0) $mom_prev_out_angle_pos(0)]  || ![EQ_is_equal $mom_out_angle_pos(1) $mom_prev_out_angle_pos(1)] }  {
            if { [string match $mom_motion_event "linear_move"] } {set flagx "ok" }
            if { [string match $mom_motion_event "rapid_move"] } {set flagx "ok" }
        }
...
global mom_current_motion
   if {[string match "*5_axis*" $mom_kin_machine_type] && \
       [DPP_GE_DETECT_HOLE_CUTTING_OPERATION] && \
       [string compare "initial_move" $mom_current_motion] && [string compare "first_move" $mom_current_motion]} {
...
#=============================================================
MOM_output_literal "(Angle_Group_list)"
MOM_output_literal "(Angle_Group_1: )"
MOM_output_literal "(A0.000)"
MOM_output_literal "#611=0(X)"
MOM_output_literal "#612=0(Y)"
MOM_output_literal "#613=0(Z)"
MOM_output_literal "(Angle_Group_2: )"
MOM_output_literal "(A0.053)"
MOM_output_literal "#621=0(X)"
MOM_output_literal "#622=0(Y)"
MOM_output_literal "#623=0(Z)"
MOM_output_literal "(Angle_Group_3: )"
MOM_output_literal "(A-180.000)"
MOM_output_literal "#631=0(X)"
MOM_output_literal "#632=0(Y)"
MOM_output_literal "#633=0(Z)"
MOM_output_literal " "
...
MOM_output_literal "G91 G28 X0.0 Y0.0"
...
#=============================================================
global mom_out_angle_pos mom_prev_out_angle_pos
global mom_nxt_motion_event mom_nxt_event

global saved_mom_out_angle_pos

if { [info exists saved_mom_out_angle_pos] } {

    if { ![EQ_is_equal $mom_out_angle_pos(0) $saved_mom_out_angle_pos(0)]  || ![EQ_is_equal $mom_out_angle_pos(1) $saved_mom_out_angle_pos(1)] }  {

        MOM_output_literal "G91 G28 Z0.0"
        PB_CMD_custom_command_2
        unset saved_mom_out_angle_pos
...
#=============================================================
proc PB_CMD_custom_command_12 { } {
#=============================================================
global flagx flagy

if { [info exists flagy ] && $flagy == "ok" } {
...
unset flagy
       ...
       global mom_cycle_retract_to_pos mom_tool_number
      MOM_output_literal "[format "G43 G00 G90 Z%.2f" $mom_cycle_retract_to_pos(2)] [format "H%d" $mom_tool_number]"
...
if {[DPP_GE_DETECT_5AXIS_TOOL_PATH]} {
    MOM_display_message "涓嶆敮鎸佷簲杞磋仈鍔?" "Info" "E"
    MOM_abort "  \n\n same tool number \n\n 娓╅Θ鎻愮ず鎮細 涓嶆敮鎸佷簲杞磋仈鍔? \n\n\n "
}
...
if { $mom_fixture_offset_value < 7 } {
    switch $mom_fixture_offset_value {
        1 { set offsetO G54 ; set offset(0) 5221 ; set offset(1) 5222 ; set offset(2) 5223 ; }
        2 { set offsetO G55 ; set offset(0) 5241 ; set offset(1) 5241 ; set offset(2) 5241 ; }
        3 { set offsetO G56 ; set offset(0) 5261 ; set offset(1) 5261 ; set offset(2) 5261 ; }
        4 { set offsetO G57 ; set offset(0) 5281 ; set offset(1) 5281 ; set offset(2) 5281 ; }
        5 { set offsetO G58 ; set offset(0) 5301 ; set offset(1) 5301 ; set offset(2) 5301 ; }
        6 { MOM_abort "G59 Is not allowed" }
    }
} else {

    set mom_fixture_offset_value [expr $mom_fixture_offset_value - 6]

    set offsetO "G54.1 P$mom_fixture_offset_value"
    set offset(0) [expr 7001 + ($mom_fixture_offset_value-1)*20]
    set offset(1) [expr 7002 + ($mom_fixture_offset_value-1)*20]
    set offset(2) [expr 7003 + ($mom_fixture_offset_value-1)*20]

}

MOM_output_literal "G90 G00 $offsetO "
MOM_output_literal "#514=0(x)"
MOM_output_literal "#515=0(y)"
MOM_output_literal "#516=0(z)"


MOM_output_literal " "
global mom_out_angle_pos
MOM_output_literal "(=================================)"
MOM_output_literal "#501=#$offset(0)-#5321"
MOM_output_literal "#502=#$offset(1)-#5322"
MOM_output_literal "#503=#$offset(2)-#5323"
MOM_output_literal "#504=[format "%.2f" [expr -1*$mom_out_angle_pos(0)]]"
MOM_output_literal "#505=[format "%.2f" $mom_out_angle_pos(1)]"

MOM_output_literal "#506=\[COS\[#505\]*#501\] + \[SIN\[#505\]*#502\]"
MOM_output_literal "#507=-\[SIN\[#505\]*#501\] + \[COS\[#505\]*#502\] "




MOM_output_literal "#508=\[COS\[#504\]*#507 - SIN\[#504\]*#503\] "
MOM_output_literal "#509=\[SIN\[#504\]*#507 + COS\[#504\]*#503\] "
MOM_output_literal "#511=#506-#501"
MOM_output_literal "#512=#508-#502"
MOM_output_literal "#513=#509-#503"
MOM_output_literal "G52 X\[#511+#514+#611\] Y\[#512+#515+#612\] Z\[#513+#516+#613\]"
MOM_output_literal "(=================================)"
...
#=============================================================
global mom_tool_number  mom_tool_name mom_tool_corner1_radius mom_tool_diameter

if { ![info exists mom_tool_corner1_radius] } { set mom_tool_corner1_radius 0}

MOM_output_literal "([format "T:%.2f" $mom_tool_number]  [format "D=%.2f" $mom_tool_diameter]  [format "R=%.2f" $mom_tool_corner1_radius] )"
}


#=============================================================
proc PB_CMD_custom_command_5 { } {
#=============================================================
global mom_current_oper_is_last_oper_in_program
global mom_next_oper_has_tool_change
if {([info exists mom_next_oper_has_tool_change] && $mom_next_oper_has_tool_change == "YES") || ([info exists mom_current_oper_is_last_oper_in_program] && $mom_current_oper_is_last_oper_in_program == "YES")} {

            global mom_tool_name mom_tool_number  mom_tool_diameter mom_tool_corner1_radius

            if { ![ info exists mom_tool_corner1_radius ] } {
            set mom_tool_corner1_radius 0
            }

 set output [format "T:%-4d ToolName:%-20s D=%-020.2f R=%-10.2f " \
                     $mom_tool_number $mom_tool_name $mom_tool_diameter $mom_tool_corner1_radius ]
...
MOM_close_output_file $ptp_file_name
MOM_remove_file ${mom_output_file_directory}temps.nc
file rename $ptp_file_name ${mom_output_file_directory}temps.nc
if { [file exists $ptp_file_name] } {
   MOM_remove_file $ptp_file_name
}
MOM_open_output_file $ptp_file_name
MOM_set_seq_off
set f [open ${mom_output_file_directory}temps.nc "r"]
set i 0
while {1} {
    incr i
    set line [gets $f]
    ...
    if { $i == 8 } {
        #鐢ㄦ埛鎺у埗杈撳嚭鍦∟C澶寸殑閮ㄥ垎鍐欏湪姝ゅ
        global mom_machine_time
        MOM_output_literal "(MachineTime=[format "%.2f" $mom_machine_time]) "
        global nod
        #MOM_output_literal "(=======Tool List=======)"
        foreach l1 $nod {
            #MOM_output_literal  "($l1)"
        }
        #MOM_output_literal "(=======Tool List=======)"
...
#=============================================================
global mom_group_name  mom_date mom_part_name
if [info exist mom_group_name] {
MOM_output_literal  "(Nc_Name:$mom_group_name)"
}
MOM_output_literal "(Company_Name:CMI)"
MOM_output_literal "(QQ:2820761766)"
MOM_output_literal "(FOR FANUC MXP-200i ONLY)"
...
if { [info exists mom_nxt_event] } {

     MOM_output_literal mom_nxt_event:$mom_nxt_event
...
set saved_mom_out_angle_pos(0) $mom_out_angle_pos(0)
set saved_mom_out_angle_pos(1) $mom_out_angle_pos(1)
...
if {[info exists flagy ] } {unset flagy }
